...
class HfRetriever(BaseRetriver):
    ...
    def encode_queries(self, 
        ...
        with torch.no_grad():
            tokenized_questions = self.question_tokenizer([query.text() for query in queries], padding=True, truncation=True, return_tensors='pt').to("cpu")
    ...
    def encode_corpus(self, 
        ...
        with torch.no_grad():
            while index < len(contexts):
                ...
                tokenized_contexts = self.context_tokenizer(samples, padding=True, truncation=True, return_tensors='pt').to("cpu")
