...
from localdata.datastructures.answer import Answer
...
from localdata.loaders.BaseDataLoader import GenericDataLoader


class WikiMultihopQADataLoader(GenericDataLoader):
    def __init__(self, dataset: str, tokenizer="bert-base-uncased", config_path='evaluation/config.ini', split=Split.TRAIN,
    ...
    def load_raw_dataset(self, split=Split.TRAIN):
        ...
        print(len(dataset))
        for  query_index, data in enumerate(tqdm.tqdm(dataset[:10])):
